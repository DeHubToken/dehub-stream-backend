...
export class StreamCronService implements OnModuleInit {
  ...
  async mainLoop(networkName) {
    ...
    if (!curNetwork) {
      this.logger.error(`Not supported network! -- ${networkName}  `);
      return;
    }
    ...
    if (diffTimestamp > 60)
      this.logger.log(
        `~${curNetwork.shortName} ---not sync graph! ${lastSyncedBlockNumber} ${Math.round(diffTimestamp)}`,
      );
    ...
    if (lastSyncedBlockNumber >= startBlockNumber) {
      ...
      this.logger.log(
        `~${curNetwork.shortName} ---fetched ${startBlockNumber} ${lastBlockFetchedForTransfer} ${lastBlockFetchedForProtocolTx} ${transfers.length} ${protocolTxes.length}`,
      );
