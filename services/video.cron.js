...
async function processingFundsForPlayingStreams() {
    let pendingStreamsForProcessing = await WatchHistory.find({ $or: [{ status: null }, { status: 'lockForPPV' }] }).lean();
    ...
    for (let i = 0; i < pendingStreamsForProcessing.length; i++) {
        ...
        else if (watchedTime > config.watchTimeForConfirming) {
            await WatchHistory.updateOne({ _id: watchStream._id }, { status: 'confirmed' });
            await Token.updateOne({ tokenId: watchStream.tokenId }, { $inc: { views: 1 } });
        }
    }
}
