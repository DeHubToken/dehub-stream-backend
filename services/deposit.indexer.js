...
async function DepositEventListener(from, tokenAddress, amount, logInfo) {
  ...
  try {
    const result = await Transaction.updateOne(
      ...
      {
        from: address,
        to: normalizeAddress(vaultContract.address),
    ...
    await Balance.updateOne(
      ...
      { $inc: { deposited: realAmount, balance: realAmount } },
      overrideOptions,
    );
  } catch (error) {
...
async function ClaimEventListener(id, tokenAddress, to, amount, timestamp, logInfo) {
  ...
  try {
    const result = await Transaction.updateOne(
      { txHash: transactionHash, logIndex, chainId },
      {
        amount: realAmount,
        from: address,
        tokenAddress: tokenAddress,
    ...
    await ClaimTransaction.updateOne(
      {
    ...
    await Balance.updateOne(
      { address, chainId, tokenAddress },
      { $inc: { claimed: realAmount, pending: -realAmount } },
      overrideOptions,
    );
  } catch (error) {
...
async function TransferEventListener(from, to, value, logInfo) {
  ...
  try {
    ...
    if (isInserted(updatedResult)) {
      await Balance.updateOne(
        { address: toAddress, chainId, tokenAddress },
        { $inc: { walletBalance: realAmount } },
        overrideOptions,
      );
      await Balance.updateOne(
        { address: fromAddress, chainId, tokenAddress },
        { $inc: { walletBalance: -realAmount } },
        overrideOptions,
      );
    }
...
async function loadHistory(tokenItem) {
  const transactionItems = await Transaction.find({
    type: 'TRANSFER',
  ...
  if (transactionItems?.length < 1 || transactionItems[0].blockNumber < latestBlock - 10) {
    ...
    while (firstBlock <= latestBlock) {
      try {
        const historyData = await getTokenHistories(
          firstBlock,
          firstBlock + limit - 1,
          tokenItem.address,
          tokenItem.chainId,
