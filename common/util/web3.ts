...
const getTokenByTokenAddress = (tokenAddress: string, chainId = 56) =>
  supportedTokens.find(e => e.address.toLowerCase() === tokenAddress.toLowerCase() && e.chainId === chainId);
...
async function multicallRead(multicallContract, callObjectArray) {
  try {
...
const getTokenBalancesOfAddresses = async (addresses, tokenAddress, chainId) => {
  ...
  const token = supportedTokens.find(
    e => e.address.toLowerCase() === tokenAddress.toLowerCase() && e.chainId === chainId,
  );
  if (!network || !token) return 0;
  const provider = new ethers.JsonRpcProvider(network.rpcUrls[0]);
  const tokenContract = new ethers.Contract(tokenAddress, erc20ContractAbi, provider);
  ...
  for (let i = 0; i < addresses.length; i += 500) {
    ...
    Object.keys(multicallResult).forEach(
      key => (multicallResult[key] = Number(ethers.formatUnits(multicallResult[key], token.decimals))),
    );
...
const getStakedAmountOfAddresses = async addresses => {
  ...
  for (let i = 0; i < addresses.length; i += 500) {
    ...
    const multicallResult = await multicallRead(multicallContract, callDataArray);
    Object.keys(multicallResult).forEach(
      key => (multicallResult[key] = Number(ethers.formatUnits(multicallResult[key], 18))),
    );
    tempResult = { ...tempResult, ...multicallResult };
  }
  return tempResult;
